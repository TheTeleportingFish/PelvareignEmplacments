...
function ENT:PhysicsCollide(data, physobj)
	...
	if velLen > (50 * 52.4934) and data.HitEntity:IsWorld() then
		sound.Play("main/ground_hit_explosive_"..math.random(1,3)..".wav", data.HitPos, 140, math.random(100,140) , 1 )
			...
			if self.PlrGrabbing:KeyDown( IN_WALK ) then
				...
				timer.Simple( 0.1 , function() 
					...
					if self:GetBodygroup( 2 ) == 0 then
						...
						Clip:EmitSound("main/grenade_clip_break_"..math.random(1,3)..".wav", 80, math.random(100,102), 1 )
		...
		if CurTime() > self.NextCharge and IsValid(self.Target) then
			...
			if Dist > 0 and SERVER then -- or Target:IsPlayer()
				...
				if TargetVelocityHighSpeed and CanBeTarget then
					...
					local SoundDistanceMax = (300 * 52.4934)
					local traceData = {}
					
					for Key, EntityInSphere in pairs( ents.FindInSphere( self:WorldSpaceCenter() , SoundDistanceMax ) ) do
						if IsValid(EntityInSphere) and (EntityInSphere:IsPlayer() or EntityInSphere:IsNPC()) then
							
							local Dist = ( 1 - (EntityInSphere:GetPos():Distance(self:GetPos())/SoundDistanceMax) )
							local SmallDist = ( 1 - (EntityInSphere:GetPos():Distance(self:GetPos())/SoundDistanceMax*2) )
							
							local ContinationWave = ((Dist)*SoundDistanceMax)/((343+math.random(-10,150)) * 52.4934)
							
							local positedplayer = EntityInSphere:WorldSpaceCenter()
							
							direction = ( positedplayer - self:WorldSpaceCenter() ):GetNormal()
							...
							local equal = (ent == EntityInSphere)
							
							timer.Simple( ContinationWave, function() 
							if IsValid(EntityInSphere) then
								if (isplayer) then
									for i = 1, 5 do
										timer.Simple( math.random(0.01,0.03) * i, function() 
											EntityInSphere:EmitSound( "main/plasma_anticharge_coniccrossover_"..math.random(1,8)..".wav", 60, math.random(40,120), Dist+math.random(-0.01,0.05), CHAN_AUTO )
											EntityInSphere:EmitSound( "main/fraiseformationshield_failure_1.wav", 90, math.random(30,50), Dist^0.4, CHAN_AUTO )
										end)
									end
									 EntityInSphere:EmitSound( "main/rigidpeircefar.wav", 75, math.random(70,80), Dist/3, CHAN_AUTO )
									
									if equal then
										EntityInSphere:EmitSound( "main/airshockwave.wav", 75, 90, Dist, CHAN_AUTO ) 
										for i = 1, 25 do
											timer.Simple( i*0.05, function() 
												EntityInSphere:SetViewPunchAngles(EntityInSphere:GetViewPunchAngles()+Angle(math.random(-10,10)*Dist, math.random(-10,10)*Dist, math.random(-10,10)*Dist)/i)
											end) 
										end
									end
									...
									if SmallDist > 0 then 
										...
										EntityInSphere:EmitSound( "main/rigidpeirce.wav", 75, math.random(70,80), SmallDist/1, CHAN_AUTO )
										
										if equal then
											EntityInSphere:EmitSound( "main/airshock.wav", 75, 100, SmallDist/20, CHAN_AUTO ) 
											EntityInSphere:EmitSound( "main/airshockwave.wav", 75, 40, SmallDist, CHAN_AUTO )
											for i = 1, 5 do
												timer.Simple( i*0.01, function() 
													EntityInSphere:SetViewPunchAngles(Angle((math.random(-5,5)/50)*(SmallDist+1)^5, (math.random(-5,5)/50)*(SmallDist+1)^5, (math.random(-5,5)/50)*(SmallDist+1)^5))
												end) 
											end
...
function ENT:CustomInitialize()
	...
	self.WindCrackle = CreateSound( self, "main/wind_breaking.wav")
