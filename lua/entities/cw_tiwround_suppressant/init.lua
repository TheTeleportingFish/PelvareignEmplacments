	...
	if IsValid(Targeted) then
		if self.CurrentDist <= self.Lowest then
			if self:GetPhysicsObject() != nil then
				local SteerForce = 5850 + math.random(-135,125)
		...
		if IsValid(Targeted) and (self.CurrentDist < (15 * 52.4934) or (DegreesBetween <= 1 and self.CurrentDist < (50 * 52.4934))) then
			...
			util.BlastDamage(self, self, self:GetPos(), 10, 10)
			...
			d:SetAttacker( self )
			d:SetInflictor( self.Owner )
			d:SetDamage( math.random(0.13,0.182) * (RoundPhys:GetVelocity():Length() / 52.4934) )
			d:SetDamageType( DMG_RADIATION )
...
timer.Simple( 0, function() 
	...
	local d = DamageInfo()
	d:SetDamage( math.random(0.063,0.15) * (Velocity:Length() / 52.4934) )
	d:SetAttacker( self )
	d:SetInflictor( self.Owner )
	d:SetDamageType( DMG_RADIATION )
		
	util.BlastDamageInfo(d, position, (5 * 52.4934))
	...
	if SERVER then
		...
		util.ScreenShake(self:GetPos(), 150, 9999, 0.1, (1 * 52.4934)) 
		
		self:SinuousExplosion( 300 )
		
	else
			
		if SERVER then
			util.ScreenShake( position, 999, 600, 0.5, (5 * 52.4934) )
		end
