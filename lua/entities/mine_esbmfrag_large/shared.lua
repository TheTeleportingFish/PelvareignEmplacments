...
ENT.PrintName = "Large ESBM Fragmentation Device"
ENT.Author = "Dog"
...
function ENT:PhysicsCollide(data, physobj)
	...
	if velLen > 100 then
		...
		if CT > self.NextImpact then
			...
			if velLen < 1000 then
				self:EmitSound("physics/metal/metal_sheet_impact_hard"..math.random(6,8)..".wav", 75, math.random(40,46))
			else
...
local Detection_Radius = (20 * 52.4934)
...
function ENT:FragmentOrCreateERBM( Entity , Amount )
	...
	for i=1, Amount do
		...
		Round:SetPos( Entity:WorldSpaceCenter() + (Entity:GetUp() + VectorRand()) * 20 )
		Round:SetAngles( AngleFire:Angle() )
		Round:Spawn()
		
		local RoundPhys = Round:GetPhysicsObject()
		RoundPhys:SetVelocityInstantaneous( (Entity:GetUp() + VectorRand()/2) *math.random(4300,5500))
	end
end
...
function ENT:SendShockWave(Pos)
	...
	local explosedist = 600 * 52.4934666667
			...
			timer.Simple( ((v:GetPos():Distance(Posed)/explosedist)^0.5 ) *0.5, function() 
				...
				for i = 1, 10 do
					timer.Simple( i*0.005, function() 
						v:SetViewPunchAngles(Angle((math.random(-18,18))*(Dist)^2, (math.random(-13,13))*(Dist)^2, (math.random(-13,13))*(Dist)^2))
					end) 
				end
...
function ENT:Think()
	...
	local tr = {}
	tr.start = self:WorldSpaceCenter() - ((ModelMaxBound.z/2) * self:GetUp())
			...
			if IsValid(Target) and SERVER then -- or Target:IsPlayer()
				...
				elseif TargetVelocityHighSpeed and CanBeTarget then
					for _, Targeted in pairs(Dmg_Array) do
						...
						local dmg_random = math.random(480000,500000) 
						...
						for i=1,25 do
							ParticleEffect( "Flashed", self:GetPos() + (VectorRand() * 50), Angle(0,0,0) )
						end
						ParticleEffect( "DirtDebrisShock", self:GetPos() , Angle(0,0,0) )
						self:FragmentOrCreateERBM( self , math.random(46,50) )
						self:Remove()
