...
function ENT:Draw()
	...
	if DistFromEntToMeters < 20 and DistFromEntToMeters > 2.5 then
		...
		if LookDist < 1 then
			...
			cam.Start3D2D( Pos , Angle, 1.5 )
				local text = "1 - 10 Meters = 500,000 - 4,800 DMG"
	...
	if DistFromEntToMeters < 2.5 then
		...
		render.SetColorMaterial()
		render.DrawLine( StartPos, self.Tracing.HitPos, Color( 255, 255*self.Tracing.Fraction^2, 255*TraceBoolConv ), true )
...
function ENT:Initialize()
	...
	hook.Add( "PostDrawTranslucentRenderables", self, function( bDrawingDepth, bDrawingSkybox )
		...
		local Pos = self:WorldSpaceCenter() + self:GetUp()*30
